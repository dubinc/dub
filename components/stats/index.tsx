...
export default function Stats({ staticDomain }: { staticDomain?: string }) {
  ...
  const {
    slug,
    domain: domainSlug,
    key = "_root", // if key is undefined, it's the root domain stats page (e.g. app.7qr.codes/steven/stey.me)
    interval = "24h",
  } = router.query as {
    slug?: string;
    domain?: string;
  ...
  const { basePath, domain, endpoint } = useMemo(() => {
    ...
    // Public stats page, e.g. 7qr.codes/stats/github, stey.me/stats/weathergpt
    return {
