...
export default function LinkCard({ props }: { props: ShortLinkProps }) {
  ...
  return (
    ...
    >
      ...
      absolute -top-1.5 right-1 flex max-w-fit cursor-help items-center space-x-1 whitespace-nowrap rounded-full border border-gray-400 bg-gray-100 px-2 py-px text-xs font-medium capitalize sm:-right-2`}
      >
        {expired ? (
        ...
        <p className="gap-5">{timeAgo(props.createdAt)}</p>
      </span>
      <button
      ...
      <div className="flex w-20 flex-col">
        ...
        <a
          ...
          rel="noreferrer"
          className={cn(
            "line-clamp-2 text-xs text-gray-600 transition-all hover:text-gray-800 ",
          )}
        >
          {props.url}
        </a>
      ...
      <button
        className={cn(
          "group rounded-full bg-gray-100 p-3 transition-all duration-75 hover:scale-110 hover:bg-blue-100 hover:text-gray-800 focus:outline-none active:scale-95",
        )}
        onClick={() => {
          setOpenPopover(false);
          setShowLinkQRModal(true);
      ...
      <a
        ...
        className="flex items-center space-x-1 rounded-md bg-gray-100 px-2 py-0.5 text-gray-500 transition-all duration-75 hover:scale-105 active:scale-100"
      >
