...
//////////// Integrations & OAuth apps starts here ////////////
...
const createIntegrations = async () => {
  const integrations = [
    {
      ...
      installUrl: "https://zapier.com/apps/dub/integrations", // use Dub OAuth flow
    },
    {
      ...
      installUrl: null, // It is Slack OAuth, so no install URL
    },
    {
      ...
      installUrl: "https://segment.com", // use Segment write key
    },
  ];
  ...
  const oAuthApps = [
    {
      ...
      clientId: "zapier-client-id",
      hashedClientSecret:
        "423e4c7eef83d7d149f4cf88bd02bdc788c5fca8c5c9ecde58daf1e83b8bc775", // "zapier-client-secret"
      partialClientSecret: "dub_app_secret_423e4c7eef",
      redirectUris: [
        "https://zapier.com/dashboard/auth/oauth/return/App204956CLIAPI/",
      ],
      pkce: true,
    },
  ];

  const integrationsCreated = await prisma.integration.createMany({
  ...
  const oauthAppsCreated = await prisma.oAuthApp.createMany({
    data: oAuthApps.map((oAuthApp) => ({
      ...oAuthApp,
    })),
  });

  console.log(`${integrationsCreated.count} integrations created.`);
  console.log(`${oauthAppsCreated.count} OAuth apps created.`);
};

//////////// Integrations & OAuth apps ends here ////////////
