...
async function main() {
  ...
  console.log(
    `Getting users created between ${startDateStr} and ${endDateStr}`,
  );
  console.log("----------------------------------------");
  ...
  const totalQrs = usersWithQrCount.reduce(
    (sum, user) => sum + user._count.qrs,
    0,
  );
  const totalLinks = usersWithQrCount.reduce(
    (sum, user) => sum + user._count.links,
    0,
  );
  const totalClicks = usersWithQrCount.reduce(
    (sum, user) =>
      sum + user.links.reduce((linkSum, link) => linkSum + link.clicks, 0),
    0,
  );
  const usersWithQrs = usersWithQrCount.filter(
    (user) => user._count.qrs > 0,
  ).length;
  const usersWithLinks = usersWithQrCount.filter(
    (user) => user._count.links > 0,
  ).length;
  const usersWithClicks = usersWithQrCount.filter(
    (user) =>
      user.links.reduce((linkSum, link) => linkSum + link.clicks, 0) > 0,
  ).length;
  ...
  const results = usersWithQrCount.map((user) => {
    const totalClicksForUser = user.links.reduce(
  ...
  console.log(
    `Users with QRs: ${usersWithQrs} (${((usersWithQrs / totalUsers) * 100).toFixed(1)}%)`,
  );
  // console.log(`Users with links: ${usersWithLinks} (${((usersWithLinks/totalUsers)*100).toFixed(1)}%)`);
  console.log(
    `Users with clicks: ${usersWithClicks} (${((usersWithClicks / totalUsers) * 100).toFixed(1)}%)`,
  );
  ...
  console.log(
    `Average clicks per user: ${(totalClicks / totalUsers).toFixed(2)}`,
  );
  if (usersWithQrs > 0) {
    console.log(
      `Average QRs per active QR user: ${(totalQrs / usersWithQrs).toFixed(2)}`,
    );
  }
  if (usersWithLinks > 0) {
    // console.log(`Average links per active link user: ${(totalLinks/usersWithLinks).toFixed(2)}`);
    console.log(
      `Average clicks per link: ${(totalClicks / totalLinks).toFixed(2)}`,
    );
  }
  if (usersWithClicks > 0) {
    console.log(
      `Average clicks per active user: ${(totalClicks / usersWithClicks).toFixed(2)}`,
    );
  }
}
