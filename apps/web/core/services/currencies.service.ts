...
import { ICurrenciesRes } from '@/app/shared/interfaces/currency.interface';
import { ICustomerBody } from '@/core/integration/payment/config';

export const getCurrenciesData = async (): Promise<ICustomerBody['currency']> => {
  const res = await ky.get<ICurrenciesRes>(
    process.env.NEXT_PUBLIC_CURRENCY_URL as string,
  );
  const { currencies } = await res.json();

  const toFloat = (value: string | number | undefined, fallback = 1): number => {
