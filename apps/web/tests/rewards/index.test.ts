...
describe.concurrent("Rewards based on partner's country", async () => {
  ...
  test("when {Partner} {Country} is {CA}", async () => {
    // Track the click
    const clickResponse = await http.post<{ clickId: string }>({
      path: "/track/click",
      headers: E2E_TRACK_CLICK_HEADERS,
  ...
  test("when {Partner} {Country} is not {US} or {CA}", async () => {
    // Track the click
    const clickResponse = await http.post<{ clickId: string }>({
      path: "/track/click",
      headers: E2E_TRACK_CLICK_HEADERS,
      body: {
        ...E2E_PARTNERS[2].shortLink,
      },
    });
