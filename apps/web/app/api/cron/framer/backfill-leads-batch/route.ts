...
export const POST = withWorkspace(async ({ req, workspace }) => {
  try {
    ...
    const existsResults = await redis.smismember(
      CACHE_KEY,
    ...
    if (payload.length > 0) {
      ...
      // throw error if some links don't have partnerId / programId
      if (links.some((l) => !l.partnerId || !l.programId)) {
        return NextResponse.json({
          message: `Links with missing partnerId or programId: ${links
            .filter((l) => !l.partnerId || !l.programId)
            .map((l) => l.key)
            .join(", ")}.`,
        });
      }
