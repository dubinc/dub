...
export const validateGroupMoveRules = (
  ...
  for (let i = 0; i < rules.length; i++) {
    ...
    if (!rule.attribute) {
      return `Rule ${i + 1}: Please select an activity.`;
    }

    // Check if value is set
    if (rule.value == null || rule.value === undefined) {
      return `Rule ${i + 1}: Please enter a value.`;
    }
    ...
    if (rule.operator === "gte") {
      if (
        ...
        return `Rule ${i + 1}: Please enter a value greater than 0.`;
      }
    }
    ...
    if (rule.operator === "between") {
      if (typeof rule.value !== "object" || rule.value === null) {
        return `Rule ${i + 1}: Please enter a valid value.`;
      }
      ...
      if (min == null || min === undefined || isNaN(min) || min <= 0) {
        return `Rule ${i + 1}: Please enter a minimum value greater than 0.`;
      }

      if (max == null || max === undefined || isNaN(max) || max <= 0) {
        return `Rule ${i + 1}: Please enter a maximum value (limit) greater than 0.`;
      }

      // Ensure max is greater than min
      if (max <= min) {
        return `Rule ${i + 1}: Maximum value must be greater than minimum value.`;
      }
    }
  }
