...
export function ApplicationFormHero({
  ...
  applicationFormData: Pick<ProgramApplicationFormData, "label" | "title" | "description">;
  preview?: boolean;
}) {
  const Heading = preview ? "div" : "h1";
  const label = applicationFormData.label || `${program.name} Affiliate Program`;
  const title = applicationFormData.title || `Apply to ${program.name}`;
  const description = applicationFormData.description 
