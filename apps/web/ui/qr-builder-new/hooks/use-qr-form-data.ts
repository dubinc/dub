...
export const useQRFormData = ({ qrType, initialData }: UseQRFormDataOptions) => {
  ...
  const parseFormData = useCallback((data: string): Record<string, any> => {
    try {
  ...
  const getDefaultValues = useCallback((overrides: Record<string, any> = {}) => {
