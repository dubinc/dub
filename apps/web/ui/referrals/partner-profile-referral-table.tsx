...
export function PartnerProfileReferralTable() {
  ...
  const status = searchParamsObj.status as string | undefined;
  ...
  const { data: totalReferralsCount, error: countError } =
    usePartnerReferralsCount<number>({
      query: {
        ...(status && {
          status: status as z.infer<
            typeof getPartnerReferralsQuerySchema
          >["status"],
        }),
        ...(search && { search }),
      },
