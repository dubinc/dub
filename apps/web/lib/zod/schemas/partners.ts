...
export const EnrolledPartnerSchema = PartnerSchema.pick({
  ...
  .extend({
    ...
    earningsPerClick: z
      .number()
      .default(0)
      .describe("Earnings per click (totalSaleAmount / totalClicks) in cents"),
    averageLifetimeValue: z
      .number()
      .nullable()
      .optional()
      .describe(
        "Average lifetime value (totalSaleAmount / totalConversions) in cents",
      ),
    clickToLeadRate: z
      .number()
      .nullable()
      .optional()
      .describe("Click to lead rate (totalLeads / totalClicks)"),
    clickToConversionRate: z
      .number()
      .nullable()
      .optional()
      .describe("Click to conversion rate (totalConversions / totalClicks)"),
    leadToConversionRate: z
      .number()
      .nullable()
      .optional()
      .describe("Lead to conversion rate (totalConversions / totalLeads)"),
    returnOnAdSpend: z
      .number()
      .nullable()
      .optional()
      .describe("Return on ad spend (totalSaleAmount / totalCommissions)"),
  })
  .merge(
