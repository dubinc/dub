...
export async function bulkCreateLinks({
  ...
  const createdLinksData = await prisma.link.findMany({
    where: {
      shortLink: {
        in: links.map((link) =>
  ...
  const finalCreatedLinks = await prisma.link.findMany({
    where: {
      id: {
  ...
  waitUntil(
    Promise.all([
      propagateBulkLinkChanges(finalCreatedLinks),
      updateLinksUsage({
