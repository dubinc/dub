...
export const parse = (req: NextRequest) => {
  ...
  // fullKey is the full path without the first slash (to account for multi-level subpaths, e.g. d.to/github/repo -> github/repo)
  // we also remove the trailing slash from the fullKey because we're doing skipTrailingSlashRedirect in next.config.js
  const fullKey = decodeURIComponent(path.slice(1)).replace(/\/$/, "");
