...
4. Navigate to the [QStash tab](https://console.upstash.com/qstash) and copy the `QSTASH_CURRENT_SIGNING_KEY`, and `QSTASH_NEXT_SIGNING_KEY` from the **Request Builder** section into your `.env` file.
...
Next, follow [these steps](https://docs.github.com/en/apps/oauth-apps/building-oauth-apps/authorizing-oauth-apps) to create a GitHub App. This will let you sign in to Dub with your GitHub account.

Don't forget to set this Callback URLs `http://localhost:8888/api/auth/callback/github` for local development.
