...
Next, you'll need to set up the MySQL database that is compatible with Planetscale. This will be used to store user data and link metadata. There are two options:

**Option 1: PlanetScale hosted database**

You can use a PlanetScale hosted database. Note they've removed the free tier, so you'll need to pay for this option.
...
**Option 2: Local MySQL database with PlanetScale simulator**

You can use a local MySQL database with a PlanetScale simulator. This is the recommended option for local development.
...
<Steps>
  ...
  <Step title="Set up database environment variables">
    ...
    **Add credentials to .env** section.
