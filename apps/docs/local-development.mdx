...
<Steps>
  ...
  <Step title="Set up database environment variables">
    ...
    <Tip>
      While we're using two different values in local development, in production or staging environments, the `PLANETSCALE_DATABASE_URL` and `DATABASE_URL` environment variables will both have the same value – the actual PlanetScale database URL.
    </Tip>
