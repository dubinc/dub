...
This step will allow you to sign in to Dub.co with your GitHub account.

First, [create a GitHub App](https://github.com/settings/apps/new). Add the following callback URL for such:

```
http://localhost:8888/api/auth/callback/github
```

For the homepage URL, you can use `http://localhost:8888/`. If you set up this correctly, you will see your GitHub profile's avatar image once you run Dub on localhost. 
...
Second, generate a `NEXTAUTH_SECRET` [here](https://generate-secret.vercel.app/32). Paste the generated value into the `.env` file. 

### Step 6: Add your Postmark API key

Add your Postmark API key to the `.env` file as the `POSTMARK_API_KEY` environment variable. 

You'll need a work email to [sign up for a Postmark account](https://account.postmarkapp.com/sign_up). 
