...
## Step 7: Set up a storage bucket

Dub stores project logos, user avatars, and images for the [Custom Social Media Cards](https://dub.co/help/article/custom-social-media-cards) feature in either S3 or S3-compatible services like [Cloudflare R2](https://cloudflare.com/r2).

We recommend using [Cloudflare R2](https://cloudflare.com/r2) for a cheaper and simploer alternative. Here is how you set it up:

<Steps>

  <Step title="Create R2 bucket">

_Note: You'll need to subscribe to the R2 service if you haven't already._

In your [Cloudflare account](https://dash.cloudflare.com/), create a new R2 bucket.
...
From the R2 main page, click "Manage R2 API Tokens" on the right-hand side. Then, click "Create API Token".

Make sure to name it something relevant to the service that will be using the token.

Give it "Object Read & Write" permissions.

And we recommend only applying ito to a single bucket.

After you create you token, copy the `Access Key ID`, `Secret Access Key` and `Endpoints for S3 clients`.
...
STORAGE_ENDPOINT={Storage Endpoint}/{Bucket Name}
```
...
Then set the `STORAGE_DOMAIN` in your `.env` file to the domain you chose.

```bash
STORAGE_DOMAIN={Your Domain}
```
