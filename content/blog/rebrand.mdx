...
Almost a year ago, we [launched Dub to the world](https://twitter.com/steventey/status/1572958186667233282). Since then, we've been growing steadily and recently surpassed the following milestones:
...
While it was great for short links, the relatively unfamiliar `.sh` TLD has been a point of confusion for many users. We've gotten feedback that the name looked suspicious – especially to a marketer who has never seen a `.sh` domain before.
...
We're very excited about the future of Dub. There are several features we're working on that we can't wait to share with you – most notably, [SAML/SSO](/help/category/saml-sso) and the [official Dub API](/help/article/dub-api).
