...
const logoSvg = `<svg width="834" height="236" viewBox="0 0 834 236" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export default function CopiableLogo({ className }: { className?: string }) {
      ...
      {createPortal(
        <>
          {contextMenu.show && (
            <div
              className={cn(
                "fixed z-50 items-center rounded-lg border border-gray-200 bg-white drop-shadow-sm",
              )}
                ...
                <Button
                  ...
                  onClick={async () => {
                    try {
                      ...
                      console.error("Failed to copy logo SVG to clipboard.", e);
                      toast.error("Failed to copy to clipboard.");
                    }
                  }}
                  icon={<Clipboard strokeWidth={2} className="h-4 w-4" />}
                  className="h-9 px-3 font-medium hover:text-gray-700"
                />
              </div>
            </div>
          )}
